...
All outputs are organized by phase under `~/Code/deep-research-output/{slug}/`.
   ...
   searches:
     - query: "{topic}"
       ...
       venues:
         ...
         acl: [2025]
   output:
     root: ~/Code/deep-research-output/{slug}/phase1_frontier/search_results
     overwrite: true
   ```

2. **Run paper_finder**: `python ~/Code/documents/tool/paper_finder/paper_finder.py --mode scrape --config phase1_frontier/paper_finder_config.yaml`
...
3. **Merge and deduplicate**:
   ```
   python ~/.agents/skills/deep-research/scripts/paper_db.py merge \
...
4. **Filter to 35-80 papers** (critical step):
   ```
   python ~/.agents/skills/deep-research/scripts/paper_db.py filter \
...
1. **Download PDFs**: `python ~/.agents/skills/deep-research/scripts/download_papers.py --jsonl paper_db.jsonl --output-dir phase3_deep_dive/papers/ --sort-by-citations --max-downloads 15`
...
4. **BibTeX**: `python ~/.agents/skills/deep-research/scripts/bibtex_manager.py --jsonl paper_db.jsonl --output phase6_report/references.bib`
5. **Compile**: `python ~/.agents/skills/deep-research/scripts/compile_report.py --topic-dir ~/Code/deep-research-output/{slug}/`
6. **Stats**: `python ~/.agents/skills/deep-research/scripts/paper_db.py stats --input paper_db.jsonl`
