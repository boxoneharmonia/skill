...
### Script Usage
```bash
python ~/.agents/skills/deep-research/scripts/search_arxiv.py \
...
### Authentication
- API key from `~/Code/keys.md` (field `S2_API_Key`)
...
### Script Usage
```bash
python ~/.agents/skills/deep-research/scripts/search_semantic_scholar.py \
...
```bash
python ~/.agents/skills/deep-research/scripts/download_papers.py \
  --jsonl paper_db.jsonl \
  --output-dir papers/ \
  --max-downloads 20 \
  --sort-by-citations

python ~/.agents/skills/deep-research/scripts/pdf_extract.py \
  --input papers/ \
  --output-dir texts/ \
  --sections
```
