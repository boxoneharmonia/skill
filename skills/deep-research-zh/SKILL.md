...
**Installation**: `~/.agents/skills/deep-research/` — scripts, references, and this skill definition.
...
**Location**: `~/.agents/skills/deep-research/scripts/search_openalex.py`
```bash
...
**Location**: `~/.agents/skills/deep-research/scripts/search_semantic_scholar.py`
Supports `--peer-reviewed-only` and `--top-conferences` filters.

### 3. search_arxiv.py (supplementary — latest preprints)
**Location**: `~/.agents/skills/deep-research/scripts/search_arxiv.py`
For searching recent papers not yet published at conferences. Mark citations with `(preprint)`.

### Other Scripts
| Script | Location | Key Flags |
|--------|----------|-----------|
| `download_papers.py` | `~/.agents/skills/deep-research/scripts/` | `--jsonl`, `--output-dir`, `--max-downloads`, `--sort-by-citations` |
| `extract_pdf.py` | `~/.agents/skills/deep-research/scripts/` | `--pdf`, `--pdf-dir`, `--output-dir`, `--sections-only` |
| `paper_db.py` | `~/.agents/skills/deep-research/scripts/` | subcommands: `merge`, `search`, `filter`, `tag`, `stats`, `add`, `export` |
| `bibtex_manager.py` | `~/.agents/skills/deep-research/scripts/` | `--jsonl`, `--output`, `--keys-only` |
| `compile_report.py` | `~/.agents/skills/deep-research/scripts/` | `--topic-dir` |
...
2. WebSearch for latest accepted paper lists
3. Identify trending directions, key breakthroughs
...
1. Write `phase2_survey/paper_finder_config.yaml` covering 2023-2026
2. Run Openalex + Semantic Scholar + arXiv
3. Merge all results: `python ~/.agents/skills/deep-research/scripts/paper_db.py merge`
4. Filter to 35-80 most relevant: `python ~/.agents/skills/deep-research/scripts/paper_db.py filter --min-score 0.80 --max-papers 70`
...
- `~/.agents/skills/deep-research/references/workflow-phases.md` — Detailed 6-phase methodology
- `~/.agents/skills/deep-research/references/note-format.md` — Note templates, BibTeX format, report structure
- `~/.agents/skills/deep-research/references/api-reference.md` — arXiv, Semantic Scholar, ar5iv API guide
